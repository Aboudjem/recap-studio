...
| `print` | `boolean` | `false` | Render for paper: the print rules apply on screen too and the document is painted light. Every page carries an `@media print` block regardless, so printing works without this. |
...
- Every page prints on a white ground with black text, fixed chrome hidden, glossary definitions expanded and each source URL spelled out. Inline SVG diagrams keep whatever colors they were authored with; the stylesheet cannot recolor an SVG's own `fill` attributes.
